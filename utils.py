...
def get_classifiers_list(MLP_path, num_classifiers=5):
    """
    Return list of intermediate MLPs. 
    
    Args: 
        MLP_path: Path of the downloaded MLPs directory.
        num_classifiers: Number of classifiers to return, default is 5.
    
    """
    classifiers_list = []
    i = 0
    for classif in sorted(os.listdir(MLP_path), key=lambda x: int(x.split('_block_')[1].split('_')[0])):
        print((classif))
        if i >= num_classifiers:
            break
        classifier = torch.load(os.path.join(MLP_path, classif)).eval().cuda()
